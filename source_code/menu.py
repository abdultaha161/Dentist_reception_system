...
def book_appointment():
    ...
    print("Appointment booked successfully for following Patient:")
    print(f"Appointment I.D={app_id} | First name= {p['first_name']} Last name= {p['last_name']}" )
...
def view_schedule():
    ...
    for a in appointments:
        if a['date'] == date:
            print(f"Time:{a['time']} | Patient ID: {a['patient_id']} | Status: {a['status']}")
            appointment_found= True
    if not appointment_found:
        print ('No record found for the following date')
